...
def build_static_system_prompt(
    ...
    sections.append(
        ...
        "Caddy running locally as a reverse proxy — the default Caddyfile at "
        "`/etc/caddy/Caddyfile` just forwards `/` to the agent on `localhost:8080`. "
        "You can edit that Caddyfile as root to expose additional services as "
        "**path prefixes on your own HTTPS domain**, e.g.:\n"
        "  ```\n"
        "  your.fqdn {\n"
        "      handle_path /myapp/* { reverse_proxy localhost:9000 }\n"
        "      handle /* { reverse_proxy localhost:8080 }\n"
        "  }\n"
        "  ```\n"
        "  Then `systemctl reload caddy` and the user can hit "
        "`https://your.fqdn/myapp/`. Keep the default `/` route pointing at 8080 so "
        "the agent stays reachable. For quick-and-dirty exposure of a raw port "
        "without touching Caddy, you can also give the user "
